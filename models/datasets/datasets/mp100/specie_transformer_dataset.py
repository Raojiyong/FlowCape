...
class SpecieTransformerPoseDataset(TransformerBaseDataset):
    ...
    def get_points_names_tokens(self):
        print("Adding point names.")
    ...
    def random_paired_samples(self):
        # 如果指定了目标类别名称列表，则只对这些类别的数据进行采样
        if self.target_class_names is not None:
            # 获取目标类别的IDs
            target_class_ids = []
            if isinstance(self.target_class_names, list):
                for class_name in self.target_class_names:
                    if class_name in self._class_to_ind:
                        target_class_ids.append(self._class_to_ind[class_name])
            else:
                # 保持向后兼容性
                if self.target_class_names in self._class_to_ind:
                    target_class_ids.append(self._class_to_ind[self.target_class_names])

            # 如果没有有效的目标类别，直接返回
            if not target_class_ids:
                self.paired_samples = np.array([])
                return

            # 收集所有目标类别的数据
            all_target_data = []
            for class_id in target_class_ids:
                all_target_data.extend(self.cat2obj[class_id])
            ...
            # 对所有目标类别的数据进行采样
            num_datas = len(all_target_data)
            all_samples = []
            # 防止无限循环，限制迭代次数
            max_iterations = max(num_datas, 1000)  # 设置一个上限
            for i in range(min(num_datas, max_iterations)):
                # 确保不会超出范围，如果数据不足则允许重复采样
                if len(all_target_data) >= self.num_shots + 1:
                    shot = random.sample(all_target_data, self.num_shots + 1)
                else:
                    # 如果数据不足，允许重复采样
                    shot = random.choices(all_target_data, k=self.num_shots + 1)
        ...
        if self.target_class_names is not None:
            ...
            if len(all_target_data) > 0:
                for _ in range(self.num_episodes):
                    # 确保不会超出范围
                    if len(all_target_data) >= self.num_shots + self.num_queries:
                        shots = random.sample(all_target_data, self.num_shots + self.num_queries)
                    else:
                        # 如果数据不足，允许重复采样
                        shots = random.choices(all_target_data, k=self.num_shots + self.num_queries)
